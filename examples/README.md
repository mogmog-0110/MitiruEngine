...
| [`model3d`](model3d/model3d_dll.cpp) | 大きな 3D モデル — 26 万ポリゴンの宮殿 (Crytek Sponza) を `drawModel` 1 行で置き、一人称で歩き回る。マウス視線 (`hud.lockMouse` + `in.mouseDeltaX`) と WASD 移動。詳細度 (LOD) は距離から自動 | ✅ |
