...
#define WIN32_LEAN_AND_MEAN   // 古い winsock.h を除外 (後続の winsock2.h と衝突しないように)
#endif
...
public:
	...
	{
		// ゲーム (host) が閉じたら、このツール窓も一緒に閉じる (逆はしない)。
		if (m_pid && !hostAlive(*m_pid))
		{
		...
		// ドック: ゲーム窓の下辺に吸着してその動きに追従する (シークバー窓)。
		if (m_dockMode != 0 && m_pid) { updateDock(); }
...
private:
	// host プロセスがまだ生きているか (閉じたらツールも閉じる)。
	static bool hostAlive(int pid)
	{
#ifdef _WIN32
		...
		CloseHandle(h);
		return r == WAIT_TIMEOUT;   // TIMEOUT = まだ動いている
#else
		(void)pid; return true;
#endif
	}

	// ゲーム窓の矩形を読み、その下辺に幅を合わせて自窓を置く (追従)。初回にフォーカスを奪わない窓へ。
	void updateDock()
	{
		auto* eng = engine();
		if (eng == nullptr || eng->window() == nullptr) { return; }
		if (!m_noActivateSet)
		{
			eng->window()->setNoActivate();   // クリックしてもゲームのフォーカスを奪わない
			m_noActivateSet = true;
		}
		if (!m_dockReader) { m_dockReader.emplace(*m_pid); }
		if (auto j = m_dockReader->poll())   // ゲーム窓の矩形 / HWND が変わったら覚える
		{
			m_gameHwnd = static_cast<std::uintptr_t>(j->value("hwnd", 0LL));
			m_gameX = j->value("x", 0); m_gameY = j->value("y", 0);
			m_gameW = j->value("w", 0); m_gameH = j->value("h", 0);
			m_haveDock = true;
		}
		if (m_haveDock && m_gameW > 0 && m_gameH > 0)
		{
			int mx = 0, my = 0, mw = 0, mh = 0;
			eng->window()->getWindowRect(mx, my, mw, mh);
			// どちらもゲーム窓の z-order の直後へ毎フレーム差し込む (間に他の窓が割り込まない)。
			if (m_dockMode == 2)
			{
				// 右脇: ゲーム窓の右辺に、自分の幅・高さのまま上端をそろえて吸着。
				eng->window()->dockBelow(m_gameHwnd, m_gameX + m_gameW, m_gameY,
				                         mw > 0 ? mw : 400, mh > 0 ? mh : 620);
			}
			else
			{
				// 下辺: ゲーム窓の真下に、ゲーム幅 × 自分の高さで吸着 (seek-bar)。
				eng->window()->dockBelow(m_gameHwnd, m_gameX, m_gameY + m_gameH,
				                         m_gameW, mh > 0 ? mh : 56);
			}
		}
	}
	...
	void ensureScrubHandler()
	{
		...
		cef->registerHandler("timetravel.resume",
			[this](std::string_view) -> std::string
			{
				// シークバーを離した: いま見ているフレームから再生を再開する。
				if (m_scrubWriter)
				{
					m_scrubWriter->write({{"resume", 1}, {"seq", ++m_scrubSeq}});
				}
				return "{}";
			});
	...
	int  m_gameX = 0, m_gameY = 0, m_gameW = 0, m_gameH = 0;   ///< ゲーム窓の画面矩形
	bool m_haveDock = false;
