 ...
 * 理由: key/value gameplay state を JS が所有し、ADR 0005 (state は C++
 * GameMemory 所有 / JS に gameplay state を持たせない) に違反。C++ StateStore と二重系統。
 * 移行先: state は C++ GameMemory + state push、表示は mitiru_bind.js (data-m-*)、
 * KV 永続化は include/mitiru/cef/StateStore.hpp。
