...
public:
	...
	void setPSConstantBuffer(UINT slot, Dx11Buffer* buffer)
	{
		...
		ID3D11Buffer* d3dBuf = buffer->getD3DBuffer();
		m_context->PSSetConstantBuffers(slot, 1, &d3dBuf);
	}
	...
	void resetScissorRect(int screenWidth, int screenHeight)
	{
		...
		rect.left = 0;
		rect.top = 0;
		rect.right = static_cast<LONG>(screenWidth);
		rect.bottom = static_cast<LONG>(screenHeight);
		m_context->RSSetScissorRects(1, &rect);
	}
