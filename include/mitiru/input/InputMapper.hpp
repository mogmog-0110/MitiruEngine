...
public:
	...
	/// @brief Bridge から one-shot action trigger を受ける
	/// @details endFrame() で flag がクリアされるまで isActionPressed() が true を返す。
	///          isActionDown() は bridge trigger では立たない
	///          (continuous press の概念が bridge には無いため)。
	/// @param actionName トリガするアクション名
	///
	/// @code
	/// mapper.triggerActionFromBridge("Jump");
	/// assert(mapper.isActionPressed("Jump") == true);
	/// mapper.endFrame();
	/// assert(mapper.isActionPressed("Jump") == false);
	/// @endcode
	...
	/// @details Native binding または bridge trigger のいずれかが有効なら true。
	///          Bridge trigger は endFrame() まで有効。
	/// @param actionName アクション名
	/// @return 今フレームで押されたなら true
	[[nodiscard]] bool isActionPressed(std::string_view actionName) const
	{
		if (m_bridgeTriggered.contains(std::string(actionName))) return true;
...
private:
	...
	/// @brief Bridge から one-shot でトリガされたアクション名の集合
	/// @details endFrame() で毎フレーム末にクリアされる。
	std::unordered_set<std::string> m_bridgeTriggered;
};
