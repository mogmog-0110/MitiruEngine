...
public:
	...
	/// @brief DisplayMode 指定版
	[[nodiscard]] std::unique_ptr<IWindow> createWindowWithMode(
		std::string_view title, int width, int height, DisplayMode mode)
	{
		auto window = std::make_unique<Win32Window>(title, width, height, mode);
		window->show();
		return window;
	}
