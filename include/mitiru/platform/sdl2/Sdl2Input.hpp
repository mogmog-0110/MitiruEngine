...
///          呼び出す。相対モーション中は SDL_MOUSEMOTION の xrel / yrel が
///          InputState::setRawMouseDelta() に蓄積される。
...
public:
	...
	///          InputState::isCursorCaptured() の現在値と前フレーム値を比較し、
	///          遷移を検出して SDL_SetRelativeMouseMode を呼び出す。
	void beginFrame() noexcept
	{
		m_state.beginFrame();
	...
	void processEvent(const SDL_Event& event)
	{
		...
		case SDL_MOUSEMOTION:
			if (m_config.enableMouse)
			{
				...
				/// 相対モーション中は xrel / yrel を生デルタとして蓄積する
				if (m_captureActive)
				{
					m_state.setRawMouseDelta(
						static_cast<float>(event.motion.xrel) * m_config.mouseSensitivity,
						static_cast<float>(event.motion.yrel) * m_config.mouseSensitivity);
				}
			}
			break;
...
private:
	/// @brief カーソルキャプチャの遷移を SDL2 に反映する
	/// @details InputState::isCursorCaptured() の値が前フレームと変わったときに
	///          SDL_SetRelativeMouseMode を呼び出す。
	///          - false → true : SDL_SetRelativeMouseMode(SDL_TRUE)
	///          - true → false : SDL_SetRelativeMouseMode(SDL_FALSE)
	///          相対モーションが拒否された場合 m_captureActive は false のままとなる。
	void applyCursorCapture() noexcept
	{
		...
		const SDL_bool sdlFlag = wantCapture ? SDL_TRUE : SDL_FALSE;
		if (SDL_SetRelativeMouseMode(sdlFlag) == 0)
		{
