...
public:
	...
	/// @brief バインドアドレスを設定する（start前に呼ぶ）
	/// @param address バインドアドレス（例: "127.0.0.1", "0.0.0.0"）
	...
	/// 公開: テスト用とプログラム的呼び出し用。本番トラフィックは start() のリッスンソケット経由で内部的にdispatchを呼ぶ。
...
private:
	...
	/// @brief クライアントソケットに受信タイムアウトを設定する
	static void setRecvTimeout(SocketHandle sock, int timeoutMs)
	{
#ifdef _WIN32
	...
	static constexpr int kRecvTimeoutMs = 5000;     ///< 受信タイムアウト（ミリ秒）
	static constexpr int kMaxRequestSize = 65536;   ///< 最大リクエストサイズ（64KB）
	...
	std::string m_bindAddress = "127.0.0.1";        ///< バインドアドレス
#ifdef _WIN32
