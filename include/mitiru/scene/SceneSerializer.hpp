...
/// @brief シーンシリアライズ/デシリアライズ（レガシー）
/// @details MitiruWorldの状態をJSON形式でシリアライズ・デシリアライズする。
///          新規コードでは SceneSerializer2 を使用すること。
///          SceneSerializer2 はコンポーネント登録・SceneGraph対応などの拡張機能を提供する。
/// @see SceneSerializer2.hpp
...
/// @brief シーンシリアライザ（レガシー）
/// @details MitiruWorldの状態をJSON形式で入出力する。
///          新規コードでは SceneSerializer2 を使用すること。
/// @deprecated SceneSerializer2 に移行してください
/// @see SceneSerializer2
///
/// @code
