...
enum class SeLimiterMode : uint8_t
{
	...
	EqualPower,   ///< 1/sqrt(n) 則でスケール (音量感を保ちつつ合算を抑制)
	Linear,       ///< 1/n 則でスケール (より保守的)
};
...
public:
		...
		float newVolume = 1.0f)
	{
		...
		for (auto& ch : m_channels)
		{
			...
			{
				fadeOutImpl(ch.handle, durationSeconds);
			}
		}
	...
	{
		MITIRU_ZONE_NAMED("Audio::Mixer::Update");
		const std::lock_guard<std::mutex> lock(m_mutex);
		for (auto& ch : m_channels)
		{
	...
	/// @param handle 再生ハンドル
	/// @return マスター × カテゴリ × SE limiter × チャンネルの積
	[[nodiscard]] float effectiveVolume(int handle) const
	{
		...
		const float limiterFactor =
			(ch->category == SoundCategory::Se)
				? computeSeLimiterFactor()
				: 1.0f;
		return m_masterVolume *
...
private:
	/// @brief カテゴリ内の全サウンドを停止する（ロック不要版）
	void stopByCategoryImpl(SoundCategory category)
	{
		for (auto& ch : m_channels)
		{
	...
	/// @brief フェードアウトを開始する（ロック不要版）
	void fadeOutImpl(int handle, float durationSeconds)
	{
		auto* ch = findChannel(handle);
		if (!ch || durationSeconds <= 0.0f)
		{
	...
	/// @brief カテゴリ内のアクティブチャンネル数を数える（ロック不要版）
	[[nodiscard]] int countActiveInCategory(SoundCategory category) const
	{
		int count = 0;
		for (const auto& ch : m_channels)
		{
	...
	[[nodiscard]] float computeSeLimiterFactor() const
	{
		...
		const int n = countActiveInCategory(SoundCategory::Se);
		if (n <= 1)
		{
			return 1.0f;
		}
		const float nf = static_cast<float>(n);
		if (m_seLimiterMode == SeLimiterMode::EqualPower)
		{
			// 1/sqrt(n): sqrt を使わず float 精度で計算
			float s = nf;
			// 単精度の簡易平方根: 標準ライブラリで十分
			s = std::sqrt(nf);
			return 1.0f / s;
		}
		return 1.0f / nf; // Linear
	}
	...
	SeLimiterMode m_seLimiterMode = SeLimiterMode::None; ///< SEクリッピング防止モード
