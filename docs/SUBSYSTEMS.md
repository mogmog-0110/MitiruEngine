...
これは [SCOPE.md](SCOPE.md) の **軸 #3「全 system 単独起動」** の deliverable であり、アトミックツール哲学 (必要なものしか画面に出さない) の engine 内 ver にあたる。Unix philosophy の「1 プログラム = 1 仕事」を engine 内部にも適用したもの。
...
各 subsystem は同じ `mitiru::Engine` を使うが、`EngineConfig::enableCef = false` で CEF を切り、gameplay 層を一切持たない。host-game 境界が本物で、game code に load-bearing でないことの証明にもなっている (ADR 0005)。
...
| replay | record / replay / diff (deterministic) | `mitiru replay --record\|--replay <file>` | `mitiru_subsys_replay.exe --record\|--replay\|--diff` |

CLI 列は別 repo `mitiru-cli` の subcommand。exe は engine の `build/examples/mitiru_subsys_<name>/` に出力され、release zip では `mitiru.exe` と並んで同梱される。終了は基本 ESC (replay は frame 数で auto-exit)。

`--diff` モードは exe 直起動でのみ提供 (CLI subcommand は `--record` / `--replay` のみ)。
...
`Engine` + 毎フレーム pull した `InputState` だけ。raw 256-key VK table を 16×16 grid で表示し (押下中は Saturn red)、mouse 座標と L/M/R button 状態、press/release を newest-first の scroll log で出す。engine が毎フレーム何を見ているかの手動 probe。**入力 plumbing 層**を isolate する。
...
### replay (`mitiru_subsys_replay`)
1 binary で 3 mode。`--record` は 300 frame の合成 InputSnapshot (右80→下60→左80→上60 frame) と RNG seed を v2 file header に記録、`--replay` は同じ stream を同じ game に再投入、`--diff <A> <B>` は 2 file を frame 単位で比較し timeline (silver=一致 / Saturn red=不一致) を描く。frame 250 で PNG screenshot を取得。InputSnapshot が game logic への唯一の入力 channel (ADR 0005) なので、record と replay の visible output は bit-exact になる。**deterministic 入力 channel** (軸 #4) を isolate する。
